...
const Video = () => {
  ...
  const { isLoading, isError, data } = useVideoDetail({
      ...
      <div className='flex flex-col lg:flex-row lg:h-screen items-stretch'>
        <div className='lg:flex-grow flex justify-center items-center relative bg-[#1E1619]'>
          <video
            ...
            src={data?.file_url}
            muted={isMuted}
            onVolumeChange={(e: SyntheticEvent<HTMLVideoElement, Event>) =>
          ...
          <div className='px-4 pt-6 pb-4 flex-shrink-0 border-b'>
            <div className='flex'>
              ...
              <div className='flex-grow'>
                <Link href={`/user/id`}>
                  <a className='font-bold block hover:underline'>{`${data?.user?.first_name} ${data?.user?.last_name}`}</a>
                </Link>
                <Link href={`/user/id`}>
